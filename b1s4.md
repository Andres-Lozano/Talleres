...
def x=tabla.map(_._1).sum.toDouble/tabla.length
def y= tabla.map(_._2).sum.toDouble/tabla.length
val numerador = tabla.map(row => (row._1 - x) * (row._2 - y)).sum
val denominadorX = math.sqrt(tabla.map(row => math.pow(row._1 - x, 2)).sum)
val denominadorY = math.sqrt(tabla.map(row => math.pow(row._2 - y, 2)).sum)
...
El coeficiente de correlación puede ser positivo o negativo dependiendo de la relación entre las dos variables; es 
positivo si ambas variables aumentan o disminuyen juntas en cambio si es negativo una variable aumenta mientras la otra disminuye.
...
va dependiendo de la relación entre el peso y la tensión sistólica dependerá del valor de 𝑟
...
Aquí, tabla.map(row => (row._1 - x) * (row._2 - y)) recorre cada tupla de la lista tabla, donde row._1 y row._2 son los valores de las variables 
 respectivamente. Para cada tupla, se calcula el producto de las diferencias entre los valores de cada tupla y sus medias correspondientes 
...
Daria lo mismo ya que son inversamente proporcionales 
